...
const FarmlandDetailsCard = ({ farmlands }) => {
    ...
    useEffect(() => {
        // Fetch weather data
        if (selectedFarmland) {
            fetch(`${process.env.REACT_APP_BACKEND_URL}/weather/getDetails/${selectedFarmland.farmlandID}`)
                .then((response) => response.json())
                .then((data) => setWeatherData(data))
                .catch((error) => console.error('Error fetching weather data:', error));

            // Fetch soil data
            fetch(`${process.env.REACT_APP_BACKEND_URL}/soil/getDetails/${selectedFarmland.farmlandID}`)
                .then((response) => response.json())
                .then((data) => setSoilData(data))
                .catch((error) => console.error('Error fetching soil data:', error));

            fetch(`${process.env.REACT_APP_BACKEND_URL}/hostcrop/getDisease/${selectedFarmland.farmlandID}`)
                .then((response) => response.json())
                .then((data) => setDiseaseData(data))
                .catch((error) => console.error('Error fetching soil data:', error));

            fetch(`${process.env.REACT_APP_BACKEND_URL}/chemicalusage/getChemical/${selectedFarmland.farmlandID}`)
                .then((response) => response.json())
                .then((data) => setChemicalData(data))
                .catch((error) => console.error('Error fetching soil data:', error));

            fetch(`${process.env.REACT_APP_BACKEND_URL}/machineryusage/getMachinery/${selectedFarmland.farmlandID}`)
                .then((response) => response.json())
