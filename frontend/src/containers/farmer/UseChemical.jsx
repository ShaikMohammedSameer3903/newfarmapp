...
const AddChemical = () => {
  ...
  useEffect(() => {
    const fetchChemicalData = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/chemical/getAll`);
        if (!response.ok) {
          throw new Error('Failed to fetch farmer data');
        }
        const data = await response.json();
  ...
  const handleAddChemical = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/chemical/addNew`, {
        method: 'POST',
        headers: {
  ...
  const handleChemicalUsageSubmit = () => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}/chemicalusage/addNew`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
